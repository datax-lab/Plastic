...
PSSM_AMINO_ACIDS = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y', '-']
LABEL_LIST = ['PHA', 'PU', 'PET', 'negative']

count_amino = 20
max_length = 1000 
class Encoding():
    def __init__(self):
        self.aaindex = pd.read_csv('./tools/aaindex_data.csv')
        pcs_x = self.aaindex.drop(['Description'], axis=1).values[:533]
        pcs_x = np.nan_to_num(pcs_x)
        pcs_x = normalize(pcs_x)
        pcs_pca = PCA(n_components=11)
        
        pcscore_matrix = pcs_pca.fit_transform(pcs_x)
        pcs_a = np.zeros((max_length - pcscore_matrix.shape[0], pcscore_matrix.shape[1]))
        self.pcscore_matrix = np.vstack([pcscore_matrix, pcs_a])

        ssc_data_list = ['BIOV880101', 'BLAM930101', 'TANS770101', 'TSAJ990101', 'NAKH920106', 'NAKH920107', 'NAKH920108',
                     'CEDJ970104', 'LIFS790101', 'LIFS790102', 'LIFS790103', 'MIYS990104', 'COWR900101', 'ARGP820101',
                     'DAWD720101', 'FAUJ880109']
        ssc_x = self.aaindex[self.aaindex['Description'].isin(ssc_data_list)]
        ssc_x = ssc_x.drop(['Description'], axis=1).values
        ssc_x = np.nan_to_num(ssc_x)
        ssc_x = normalize(ssc_x)
        ssc_pca = PCA(n_components=11)

        ssc_matrix = ssc_pca.fit_transform(ssc_x)
        ssc_a = np.zeros((max_length - ssc_matrix.shape[0], ssc_matrix.shape[1]))

        self.ssc_matrix = np.vstack([ssc_matrix, ssc_a])
    ...
    def zScales(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 5), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def VHSE(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 8), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def PCscores(self, data):
        return self.pcscore_matrix

    def sScales(self, data):
        return self.ssc_matrix


    def one_hot_encoding(self, data):  # same Identity descriptor
        matrix = np.zeros((max_length, count_amino), dtype=np.float64)
        count = 0
        for i in data:
            if count >= max_length:
                break
    ...
    def six_bit_encoding(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 6), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
                matrix_np[count] = six_bit_amino_acid[i]
                count += 1
        return matrix_np
    ...
    def hydrohobicity_matrix(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 20), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def meiler_parameters(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 7), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def acthely_factors(self, data):
        ...
        for i in data:
            if count >= max_length:
                ...
                matrix_np[count] = acthely_factors_list[i]
                count += 1
        return matrix_np
    ...
    def aesnn3(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 3), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def ann4d(self, data):
        ...
        count = 0
        matrix_np = np.zeros((max_length, 4), dtype=np.float64)
        for i in data:
            if count >= max_length:
                break
            else:
    ...
    def protvec(self, data):
        ...
        import json
        with open('/home/kyung/Plastic/plastic_model/tools/plastic_protvec.json', 'r') as f:
            json_data = json.load(f)

        count = 0
        matrix_np = np.zeros((max_length, 100), dtype=np.float64)
